...
export function SignUpForm({ onLoad }: Readonly<{ onLoad: Function }>) {
	const auth = useAuth();
	...
	const onAuth = React.useCallback(async (providerId: string) => {
		setIsPending(true);
	...
	return (
		<Stack spacing={4}>
			<div>
				<Box component={RouterLink} href={paths.home} sx={{ display: "flex", fontSize: 0 }}>
